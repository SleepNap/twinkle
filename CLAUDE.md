...
按 `ARCHITECTURE.md` 第十一节推进：M0（骨架+基础验证+热更新地基）→ M1（协议+Netty）→ M2（游戏逻辑重写，参考项目作 parity 真值）→ M3（HTTP+AI+渐进重载）→ M4（插件+热更新 L1-L4）→ M5（Web 控制台+迁移）→ M6（分布式）。**M0-M5 已完成**（M5 后端面 2026-08-09，Web 控制台前端页面未做，见任务文档"完成范围诚实标注"），当前应聚焦 **M6**。各里程碑完成范围与遗留项见 `twinkle-server/tasks/` 下对应任务文档的"完成范围诚实标注"节。**事故报告阶段 B 已完成**（2026-08-09，`docs/ghost-player-monster-controller-incident.md`）：分阶段心跳 + 会话代际 + 怪物控制租约三块稳定层落地，见项目记忆"幽灵玩家事故报告阶段 B 完成"节。
