...
**Web 控制台已进入正式业务开发**（2026-08-12）：`twinkle-web/` 已落地 shadcn `radix-nova` 控制台框架、路由、管理 API 层，以及运行概览、频道、在线玩家、账号角色、配置中心、运维操作、API Key、能力目录、审计日志和任务监控页面；配置热改、踢下线、脚本/逻辑重载、重启、API Key 生命周期与 Scope 调整均已接入确认和反馈。能力目录按当前 API Key Scope 裁剪 Tool，展示详情与 JSON Schema，并提供公开 OpenAPI 下载。统一 `BackgroundTaskRegistry` 已提供有界执行历史、调度启停、立即运行与失败重试，`AiDailySummaryScheduler` 为首个真实接入任务。完整范围见 `docs/in-progress/console-roadmap.md`。**控制台强鉴权 + RBAC + 不可抵赖审计已落地**（2026-08-18，安全顺序 1/2/3）：`/admin/v1` 挂 `AdminAuthFilter`（账号 BCrypt 登录 + DB session token），可配置角色表（`admin_role`/`account_admin_role`）+ 写操作 reason 审计（`admin_operation_audit`），前端登录页 + `roles-page` 闭环。AI 预算/模型策略已于 2026-08-19 完成；后续主线为任务持久化与集群聚合、角色高风险操作评审和部署。

**AI 计费两处存量缺陷已修复**（2026-08-19，4.2 前置止血）：①`model_rate.model_key` 用裸 modelName、计费传 descriptor，外部模型一路静默免费（V19 迁移修正口径）；②`/api/v1/ai/chat` 绕过能力面完全不计费（计费下沉 core `AiGovernanceService` 契约，唯一计费点收敛到 `AiFacade.investigate`，三条入口全覆盖）。回归锁在 `AiBillingE2ETest`。

**控制台 4.2「AI 权限与预算」已完成**（2026-08-19）：策略挂**账号维度**（`account_records.id`，与积分账户同口径）——`subjectId` 继承签发者会塌缩成"全体一份"，弃用。`AiPolicyService`（http-api）分层判定：运行期全局开关 `ai.runtime.enabled` → 全局模型白名单 `ai.allowed.models` → 解析计费账号 → 账号策略（`ai_usage_policy` 表，日调用/积分/Token 上限，`>0` 生效 `0` 不限，24h 滚动窗口，无策略行=不限制）→ 余额与订阅计划。**前两步必须在解析计费账号之前**（管理员凭据在解析处短路返回免计费，判定顺序反了全局策略对它就失效）。管理员凭据保留免计费但受全局策略约束。拒绝码按性质分流：额度类 429、`ai_disabled` 503、策略类 403 不可重试。顺带回填 `ai_usage_log` 的 `points_cost`/`account_id`（此前恒 0）并补索引。契约 `/admin/v1/ai/{status,policies,usage}` + 权限点 `admin.ai:manage` + 控制台 `/ai-policies` 页。回归锁 `AiPolicyE2ETest` / `AiPolicyServiceTest`。详见 `docs/in-progress/console-roadmap.md` 4.2 节（含诚实标注）。
...
- **AI 计费与策略唯一挂载点 = `AiFacade.investigate`**（2026-08-19 收敛）：AI 入口有三条（能力面 `server.agent.investigate` / `/api/v1/ai/chat` / 游戏内 `@gm`），治理走 core 契约 `AiGovernanceService`（`precheck(subjectId, credentialId, modelDescriptor)` / `settle(...)` 返回扣除积分数，实现 `BillingAiGovernance` 在 http-api，`NoopAiGovernanceService` 兜底）。**新增 AI 入口不要各自接计费或策略**，也不要在 `ToolExecutionService` 里恢复扣费（会双扣）。模型标识统一 descriptor（`provider/modelName`），与 `model_rate.model_key` 同口径。AI 运行态（连通性/降级/最近错误）走另一条 core 契约 `AgentStatusService`——管理面不依赖 ai 模块。
