...
- 每个频道的游戏状态由稳定 `GameExecution` 串行执行；封包、频道 Tick、脚本、断线、管理快照和异步状态回调遵守同一执行归属。DB 在频道内取快照后交单写队列，禁止在频道任务中同步等待存档。可重载逻辑隔离在**模块 classloader**，状态不进该 classloader。
...
### 并发与状态访问（用户补充的硬约束，2026-09-06）

当前实现与剩余边界见 `ARCHITECTURE.md` 第 5.1、5.4 节及 [并发整改进度](docs/in-progress/gameplay-roadmap.md#并发入口整改2026-09-06)。新增封包必须复用频道执行入口；异步逻辑回调用 `continueAt` 携带版本，稳定交接回调须复核会话和交接令牌。

**先收口共享状态的访问入口和执行归属，再在必要边界加锁。** 参考北斗仅用于理解协议与行为，不沿用其分散加锁方式；不得以参考项目已有大量锁为本项目继续堆锁的理由。

1. **每份共享可变状态必须有明确归属**：说明由哪个执行上下文负责修改、哪些入口可以访问。优先按业务边界串行处理；封包、Tick、脚本、管理操作及异步回调对同一份状态的修改必须进入同一套协调机制。串行范围按角色、地图或频道的实际一致性需求确定，禁止用全服大锁代替边界设计。
2. **业务操作的并发责任只由明确的一层承担**：外部调用业务入口，不需要自行补锁；同一操作的校验、计算和提交应处于完整的一致性边界内。禁止在 Handler → Service → System → 实体调用链中，因职责不清而逐层重复锁同一对象，也不能只锁最后的写入而放任前置校验失效。
3. **读写都必须遵守同一规则**：跨线程读取使用在所属执行上下文或同一把锁内生成的一致、不可变快照；不得泄漏可变集合或实例让调用方绕过入口修改。`volatile`、并发集合、单个方法加锁和版本门都不能单独保证整个业务操作的原子性；版本门仍须保留，用于拒绝旧逻辑的迟到写入。
4. **确需互斥时默认使用少量、边界明确的 `synchronized`**：只有需要尝试获取、限时或可中断等待、多个条件队列、公平性等具体能力时才选 `Lock`，并保证释放。不得以笼统的“性能更好”为由替换锁，也不得为减少锁的数量直接删除已有保护。锁和执行队列随稳定状态持有，不随可替换逻辑换代而失去协调。
5. **持锁期间禁止慢操作和不可控调用**：数据库、文件、阻塞网络 I/O、等待异步结果及外部回调不得放进状态锁内。存档先在一致性边界内取得独立快照及其脏版本，再交后台写库；成功后按对应版本清理脏标记，避免覆盖期间的新修改。封包和事件发送应保持状态提交顺序，移出锁时必须同时保证顺序。
