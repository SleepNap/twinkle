...
## 2026-08-21 已完成增量（4.1 账号管理与角色只读详情）

- `/admin/v1/accounts` 已支持账号名模糊搜索、状态筛选和 offset/limit 分页。
- 新增账号详情与所有世界的角色快照，不再要求控制台额外配置能力面 API Key。
- 新增封禁/解封、禁言/解禁、账号级强制下线与残留 `logged_in` 修复；封禁会同步踢下在线角色。
- 写操作使用独立权限 `admin.account:manage`，强制填写 reason，并记录变更前后摘要与 requestId。
- 前端 `/accounts` 已按 shadcn `radix-nova` 组件与语义令牌重做。
- 新增角色持久化详情聚合：基础属性、位置、公会/队伍关联、货币、背包/装备、任务进度、技能和好友；控制台以标签页展示，并明确不包含在线内存中尚未落库的数据。
- 新增 `/capabilities` 能力目录页：按当前 API Key 的 Scope 展示真实可见 Tool，支持检索、契约详情、输入/输出 JSON Schema 和公开 OpenAPI 下载。
...
| 账号角色 | 分页检索、账号管控、跨世界角色与关联数据快照 | `/admin/v1/accounts*` |
...
| 能力目录 | 按凭据裁剪的 Tool 检索、详情、Schema、OpenAPI 下载 | `/api/v1/capabilities*`、`openapi.yaml` |
...
### 4.1 账号与角色管理（账号管控与只读详情已完成）

- ~~账号分页、模糊搜索和筛选，而非仅支持完整账号名。~~ ✅
- 封禁/解封、禁言、强制下线、登录状态修复 ✅。
- 角色详情、装备、背包、任务、技能、好友、公会/队伍关联、位置和货币快照 ✅（完整公会资料需后续独立契约）。
- 受审计保护的角色修复、传送、发放物品和货币调整。
- 当前账号写操作已记录操作者、原因、变更前后摘要和 requestId ✅；后续角色写操作沿用同一规则。

临时封禁暂不开放：`account_records.temp_ban` 虽有字段，但当前登录校验没有执行该字段。必须先补登录期判定、时区/过期语义和回归测试，再暴露控制台写入口，避免生成无实际效果的管控状态。

已落地契约：

```text
GET  /admin/v1/accounts?query=&status=&offset=&limit=
GET  /admin/v1/accounts/{accountId}
GET  /admin/v1/accounts/{accountId}/characters/{characterId}
PUT  /admin/v1/accounts/{accountId}/restrictions
POST /admin/v1/accounts/{accountId}/force-offline
```
...
4. ~~任务注册表与统一状态机基础版。~~ ✅ 已完成；持久化与集群聚合留下一阶段。
5. 账号管控与角色只读详情已完成；继续评审角色修复、内容管理和运营功能。

`/admin/v1` 已套强鉴权（`AdminAuthFilter`），4.2（AI 预算/策略）与 4.1 账号管控、角色只读详情已完成。
对外暴露前仍需按实际运营范围评审角色修复、物品/货币发放等高风险能力是否必须上线。
