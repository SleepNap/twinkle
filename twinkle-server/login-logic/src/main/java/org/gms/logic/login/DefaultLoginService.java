 ...
 *
 * <p>当前部署为单 world，world ID 由配置决定且不要求为 0。密码用 BCrypt 散列校验（不落明文）。
 *
...
public final class DefaultLoginService implements LoginService {
     ...
     */
    public DefaultLoginService(GameAccountRepository accountRepository, PlayerCharacterRepository characterRepository,
    ...
    public LoginResult authenticate(String name, String password) {
        Optional<GameAccountRecord> acc = accountRepository.findByName(name);
        if (acc.isEmpty()) {
            return LoginResult.error(5); // 账号不存在
        }
        GameAccountRecord account = acc.get();
     ...
     *
     * <p>校验规则（思路参考 BeiDou-Server PlayerCharacterRecord.canCreateChar）：
    ...
    public PlayerCharacterRecord createCharacter(long accountId, int world, String name, int job, int face,
        ...
        PlayerCharacterRecord chr = new PlayerCharacterRecord();
    ...
    private void insertDefaultEquip(PlayerCharacterRecord chr, int itemId, int position) {
        if (itemId <= 0) {
            return;
        }
