...
import org.gms.httpapi.api.v1.mapper.CapabilityV1Mapper;
...
public final class CapabilityController {
    ...
    @Operation(summary = "查询可见 Tool 能力", responses = {
            @ApiResponse(responseCode = "200", content = @Content(
                    schema = @Schema(implementation = CapabilityCatalogResponse.class))),
    ...
    public HttpResponse<?> capabilities(HttpRequest<?> request,
        ...
        try {
            return HttpResponse.ok(CapabilityV1Mapper.catalog(
                            toolCatalogService.catalog(principal, profile, query)))
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(summary = "查询 Tool 能力详情", responses = {
            @ApiResponse(responseCode = "200", content = @Content(
                    schema = @Schema(implementation = CapabilityDetailResponse.class))),
    ...
    public HttpResponse<?> capability(HttpRequest<?> request, @PathVariable String toolId) {
        return toolCatalogService.detail(principal(request), toolId)
                .<HttpResponse<?>>map(detail -> HttpResponse.ok(
                        CapabilityV1Mapper.detail(detail)))
    ...
    public String openApi() {
        return contractService.openApi(1);
    }
