 ...
 */
public final class DefaultItemSystem implements ItemSystem {
    ...
    public DefaultItemSystem(VersionGate versionGate, GameDataProvider gameData) {
    ...
    public boolean giveItem(CharacterState state, int itemId, int quantity) {
        ...
        if (itemId / 1000000 == 1) {
            ...
            synchronized (state) {
                ...
                return state.addTradeItems(Collections.nCopies(quantity, DefaultEquipmentSystem.createItem(data)), Map.of(itemId, 1));
            }
        }
