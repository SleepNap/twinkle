 ...
 *
 * <p>纯转换：从 data.PlayerCharacter（74 列标量）读出，填入 domain.PlayerCharacter（内存态权威，
 ...
 */
public final class PlayerCharacterAssembler {
    ...
    public PlayerCharacterAssembler(VersionGate versionGate) {
        this(versionGate, null, null, null);
    }

    public PlayerCharacterAssembler(VersionGate versionGate, InventoryItemRepository inventoryItemRepository) {
    ...
    /** data.PlayerCharacter（DB 存档）→ domain.PlayerCharacter（内存态角色）。 */
    public PlayerCharacter fromData(org.gms.persistence.entity.PlayerCharacterRecord db) {
        ...
        chr.restoreRewardReceipts(RewardReceiptCodec.decode(db.getRewardReceipts()));
        chr.setLevel(db.getLevel());
        chr.setExp(db.getExp());
        chr.setGachaExp(db.getGachaExp());
        chr.setStrStat(db.getStrStat());
        chr.setDexStat(db.getDexStat());
        chr.setLukStat(db.getLukStat());
    ...
    private void loadQuests(PlayerCharacter chr, long characterId) {
    ...
    private static Equip toEquip(InventoryItemEntity e) {
        ...
        equip.setStrStat((short) e.getStrStat());
        equip.setDexStat((short) e.getDexStat());
        equip.setIntStat((short) e.getIntStat());
        equip.setLukStat((short) e.getLukStat());
    ...
    /** 构建全部五类背包的完整数据库快照。 */
    public List<InventoryItemEntity> toInventoryData(PlayerCharacter chr) {
    ...
    /** 构建全部任务状态的数据库快照。 */
    public List<QuestStatusEntity> toQuestStatusData(PlayerCharacter chr) {
    ...
    public List<QuestProgressSnapshot> toQuestProgressData(PlayerCharacter chr) {
    ...
    /** 构建全部技能的数据库快照。 */
    public List<SkillEntity> toSkillData(PlayerCharacter chr) {
    ...
    private static void copyEquipStats(Equip equip, InventoryItemEntity entity) {
        ...
        entity.setStrStat(equip.getStrStat());
        entity.setDexStat(equip.getDexStat());
        entity.setIntStat(equip.getIntStat());
        entity.setLukStat(equip.getLukStat());
    ...
    /** domain.PlayerCharacter（内存态权威）→ data.PlayerCharacter（DB 存档，L4 增量 FLUSH 落库用）。 */
    public org.gms.persistence.entity.PlayerCharacterRecord toData(PlayerCharacter chr) {
        org.gms.persistence.entity.PlayerCharacterRecord db = new org.gms.persistence.entity.PlayerCharacterRecord();
        db.setId(chr.getId());
        ...
        db.setRewardReceipts(RewardReceiptCodec.encode(chr.rewardReceipts()));
        db.setLevel(chr.getLevel());
        db.setExp(chr.getExp());
        db.setGachaExp(chr.getGachaExp());
        db.setStrStat(chr.getStrStat());
        db.setDexStat(chr.getDexStat());
        db.setLukStat(chr.getLukStat());
