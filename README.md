...
> 项目目前处于积极开发阶段，适合参与开发、架构研究和测试验证，尚未作为“下载即开服”的稳定发行版交付。当前优先迁移游戏封包与补齐玩法，真实 v83 客户端联调和完整场景验收后续统一进行。

## 项目特点

- **v83 协议栈**：基于 Netty 实现登录服、频道服、封包编解码、加解密与 Handler 注册机制，自动化测试已覆盖握手、登录、选角和进图主链路。
- **分层热更新**：已有动态配置、外部 JavaScript 重载和插件装卸基础；内置 Java 游戏逻辑的完整制品替换与调用切换尚待实现。修改前可查 [热更新开发者指南](docs/HOT-UPDATE.md)，区分当前能力与目标边界。
...
截至 2026-09-06，已接入四批游戏功能迁移，最新一批为装备穿脱与属性。接下来推进战斗与死亡结算；迁移期间保留编译、相关自动回归和架构检查，客户端联调后续统一安排。

| 范围 | 状态 |
| --- | --- |
| 工程底座、数据层、迁移器与热更新基础 | 已实现 |
| v83 协议、登录/选角/进图自动化链路 | 已接入并有自动回归，真实客户端待统一验收 |
| 基础游戏交互 | 已接入普通换图、背包与拾取、普通 NPC 商店、AP/SP、基础任务、三个自身 Buff 和同频道临时队伍 |
| 同屏角色与操作设置 | 已接入角色生成/移除、移动/外观、表情/椅子、同屏 Buff、角色详情、键位/宏/快捷栏存档、队伍聊天和手动金币丢弃/拾取 |
...
游戏功能的批次记录、实现边界、自动验证结果与后续路线见 [游戏路线图](docs/in-progress/gameplay-roadmap.md)。控制台范围见 [控制台路线图](docs/in-progress/console-roadmap.md)，历史基础架构里程碑见 [任务归档](docs/archived/tasks/README.md)。
